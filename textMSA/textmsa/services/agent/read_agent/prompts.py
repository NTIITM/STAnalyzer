...
PLAN_PROMPT = {
    "en": """
You are a file reading planning assistant. Based on the user question and file tree, create a SEQUENTIAL reading plan that lists all files that need to be processed in a specific order.

User question:
{user_query}

File tree:
{file_tree_format_string}
...
2. **Plan structure:**
   - Each plan item should specify: file_id, file_name, file_path, plan_detail, and order_reasoning
...
{{
  "plans": [
    {{
      "file_id": "actual file_id from the file tree",
...
**Important Notes:**
- file_id must be extracted from the file tree, each file node contains file_id information (format: "file_id: xxx")
- Do not use placeholders or example values, must use the actual file_id shown in the file tree
...
""",
    "zh": """
你是文件阅读规划助手。根据用户问题和文件树，制定顺序阅读计划，按特定顺序列出所有需要处理的文件。
...
2. **计划结构：**
   - 每个计划项应指定：file_id, file_name, file_path, plan_detail, 和 order_reasoning
...
**重要提示：**
- file_id 必须从文件树中获取，每个文件节点都包含 file_id 信息（格式为 "file_id: xxx"）
- 不要使用占位符或示例值，必须使用文件树中显示的实际 file_id
...
1. For all other questions, generate a narrative final answer that:
   - Is a coherent narrative text based entirely on the execution results
   - Directly answers the user's question using information from the execution results
   - References specific files using their file names when mentioning results
   - Provides a comprehensive analysis of all reading results
   - Does NOT include any suggestions, recommendations, or advice beyond what is in the execution results
   - Is based solely on the interpretation of the execution results
...
1. 对于所有其他问题，生成叙述性的最终答案，要求：
   - 是一段连贯的叙述性文字，完全基于执行结果
   - 使用执行结果中的信息直接回答用户的问题
   - 在提及结果时，使用文件名引用特定文件
   - 对所有读取结果进行综合分析
   - 不包含任何建议、推荐或超出执行结果范围的建议
...
def format_data_preview_analysis_prompt(
    ...
    if not previous_results:
        lang = _normalize_language(language)
        previous_results = "尚未读取任何文件。" if lang == "zh" else "No previous files have been read yet."
...
def format_code_generation_prompt(
    ...
    """格式化代码生成 prompt"""
    if not previous_results:
        previous_results = "No previous files have been read yet."
    lang = _normalize_language(language)
    if not analysis_guidance:
        analysis_guidance = "无分析指导信息可用。" if lang == "zh" else "No analysis guidance available."
...
**Answer Requirements:**
- Your answer must strictly adhere to the instruction and only report what is found in the file content and execution results
- Use natural narrative text, not lists or structured formats
- Do NOT include any suggestions, recommendations, or advice beyond what is in the execution results
- Do NOT provide any recommendations or suggestions that go beyond the scope of the execution results

Please provide a clear and concise answer based on the file content and considering the context from previously read files.
""",
    "zh": """
...
**回答要求：**
- 你的回答必须严格忠于指令，只报告文件内容和执行结果中发现的内容
- 使用自然的叙述性文字，不要使用列表或结构化格式
- 不包含任何建议、推荐或超出执行结果范围的建议
- 不要提供任何超出执行结果范围的推荐或建议

请基于文件内容并提供简洁明了的回答，同时考虑之前读取文件的上下文。
""",
}
