...
class GraphST():
    def __init__(self, 
        ...
        n_neighbors : int, optional
            Number of neighbors to use when constructing the spatial interaction graph. The default is 6.
        Returns
        -------
        ...
        if 'adj' not in adata.obsm.keys():
           if self.datatype in ['Stereo', 'Slide']:
              construct_interaction_KNN(self.adata, n_neighbors=self.n_neighbors)
           else:    
              construct_interaction(self.adata, n_neighbors=self.n_neighbors)
