...
def zscore(matrix: Union[np.ndarray, csr_matrix],
    ...
    # Variance should never be negative, but floating point errors can cause it
    variance = np.maximum(variance, 0.0)
    
    # Avoid division by zero: when variance is 0 or very small, set it to 1 to avoid inf
    # This happens when all values in a column are the same
    # Use a small epsilon to handle near-zero variance
    epsilon = 1e-10
    variance = np.where(variance < epsilon, 1.0, variance)
    
    # Compute z-score with safe division
    sqrt_variance = np.sqrt(variance)
    zscored_matrix = (matrix - E_x) / sqrt_variance
    ...
    # nan_to_num converts both NaN and inf to finite numbers
    zscored_matrix = np.nan_to_num(zscored_matrix, nan=0.0, posinf=0.0, neginf=0.0)
    
    # Additional safety check: verify no invalid values remain
    if np.any(np.isnan(zscored_matrix)) or np.any(np.isinf(zscored_matrix)):
        # If still have invalid values, replace with zeros
        zscored_matrix = np.nan_to_num(zscored_matrix, nan=0.0, posinf=0.0, neginf=0.0)
