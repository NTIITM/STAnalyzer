...
def pca_umap(banksy_dict: dict,
    ...
    for nbr_weight_decay in banksy_dict:
        ...
        for lambda_param in banksy_dict[nbr_weight_decay]:
            ...
            # Ensure X is a numpy array (not matrix)
            if isinstance(X, np.matrix):
                X = np.asarray(X)
            
            # Handle NaN and inf values before PCA
            # PCA cannot handle NaN or inf values
            # First check for invalid values
            if np.any(np.isnan(X)) or np.any(np.isinf(X)):
                print(f"Warning: Found NaN or Inf values in matrix. Replacing with 0.0")
                X = np.nan_to_num(X, nan=0.0, posinf=0.0, neginf=0.0)
            
            # Additional safety: ensure all values are finite
            X = np.nan_to_num(X, nan=0.0, posinf=0.0, neginf=0.0)
            
            # Verify no invalid values remain
            if np.any(~np.isfinite(X)):
                print(f"Warning: Still found non-finite values after cleaning. Replacing with 0.0")
